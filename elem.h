...
{
  friend class htable;
 protected:  // these will change depending
           // on the client needs

  int key;      // area code
  string team_name;  // team name
  string park_name; // ballpark name
  string location;  // location of the ballpark
  int capacity;  // capacity of the ballpark
  string roof; // roof for the ballpark

 public:

  el_t();  // to create a blank el_t object
  el_t(int, string, string, string, int, string); // to create an initialized el_t object - useful for the client
